...
label {
  ...
  margin-bottom: 8px;
   ...
   background-color:#800000;
   text-align: center;
  color: white; /* rouge pour les étiquettes */
...
select {
  ...
  margin-bottom: 20px;
  padding: 10px;
  border: 1px solid #fff; /* rouge pour les bordures */
...
input[type="submit"] {
  ...
  height: 40px;
  margin-bottom: 20px;
  background-color: #800000; /* rouge pour le bouton */
...
input[type="submit"]:hover {
  background-color: #400000; /* gris pour le bouton lors du survol */
}
...
ul.liste li {
  padding: 20px;
  border-bottom: 1px solid #800000; /* rouge pour les séparateurs */
}
...
#charCount {
  font-size: 12px;
  color: #fff; /* rouge pour le compteur de caractères */
  margin-bottom: 10px;
}
...
<?php
    if (isset($_GET['error'])) {
        echo "<p>veuillez selectionnez la plateforme elle existe</p>";
      }
      ?>
</head>
<body>
<svg height="171.4" width="400" viewBox="0 0 400 171.4">
    ...
    <?php  $conn = new mysqli("localhost","root", "","gamemania" );
...
<form action="upload_abo.php" method="POST" enctype="multipart/form-data">
...
<h2> veuillez remplire les informations selon les informations du constructeur</h2>
  ...
  <label for="Platforme">selectionnez la platforme de l'abonnement si elle n'existe pas cocher la case et inserez une nouvelle </label><br>
  <select name="Platforme" id="Platforme">
  <option value="1" disabled>Platforme</option>
  
  <?php

$sql = "SELECT  DISTINCT  IDP,nom_plateforme FROM plateforme_abonnement WHERE '1'";
...
?>
  ...
  <input type="checkbox" id="myCheckboxp" name="myCheckboxp"  onclick="toggleText2()"><br>
   <p id="textp" style="display:none;"><label for="nomp" >Nom Platforme:</label><br>
  <input type="text"  id="nomp" name="nomp" ><br></p>

  <div style="display: flex;">
  <label for="format" style="margin-right:30px;">Selectionnez le format du delai :</label>
...
<label>nombre de jours\mois  </label>
<input type="number" name="delait" max="30" min="1" default="-1" required/><br>

  <label >Montant de la solde</label>
<input  type="range" min="0" max="100" step="5" value="0" name="solde" id="solde">
<div id="rangeValue" style="color: white;"> 0</div>
...
function toggleText2() {
    var checkBox = document.getElementById("myCheckboxp");
